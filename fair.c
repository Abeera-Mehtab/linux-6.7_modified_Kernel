...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	/* MODIFICATION START */
	if (entity_is_task(se)) {                               // [4] Ensure se is a real task
		struct task_struct *task = task_of(se);             // [5] Get task struct safely
		kuid_t euid = task_euid(task);                       // [6] Extract effective UID	
			
		if (uid_eq(euid, GLOBAL_ROOT_UID)) {                // [7] If root user
			vruntime -= 1000000ULL;                         // [8] Prioritize root tasks
		}
	}
	 ...
	 */
	if (sched_feat(PLACE_DEADLINE_INITIAL) && (flags & ENQUEUE_INITIAL)) //added flag
		vslice /= 2;
	/*
	 * EEVDF: vd_i = ve_i + r_i/w_i
	 */
